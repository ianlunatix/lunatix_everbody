...
clear
echo "IP=$domain" > /var/lib/LT/ipvps.conf
if [[ "$IP" = "" ]]; then
...
cat <<EOF> /etc/systemd/system//etc/nginx/conf.d/vps.conf
server {
  listen       81;
...
wget -q -O xraymode.sh https://raw.githubusercontent.com/ianlunatix/lunatix_everbody/main/xray/xraymode.sh && chmod +x xraymode.sh && ./xraymode.sh
sleep 1 

# config.json
wget -q -O /etc/xray/config.json "https://raw.githubusercontent.com/ianlunatix/lunatix_everbody/main/config/config.json"
chmod +x /etc/xray/config.json

# vme.json
wget -q -O /etc/xray/vme.json "https://raw.githubusercontent.com/ianlunatix/lunatix_everbody/main/config/vme.json"
chmod +x /etc/xray/vme.json

# vle.json
wget -q -O /etc/xray/vle.json "https://raw.githubusercontent.com/ianlunatix/lunatix_everbody/main/config/vle.json"
chmod +x /etc/xray/vle.json

# tro.json
wget -q -O /etc/xray/tro.json "https://raw.githubusercontent.com/ianlunatix/lunatix_everbody/main/config/tro.json"
chmod +x /etc/xray/tro.json

sleep 1

# xray.conf
rm -f /etc/nginx/conf.d/xray.conf
wget -q -O /etc/nginx/conf.d/xray.conf "https://raw.githubusercontent.com/ianlunatix/lunatix_everbody/main/config/xray.conf"
...
systemctl enable vmejs >/dev/null 2>&1
systemctl start vmejs >/dev/null 2>&1
systemctl restart vmejs >/dev/null 2>&1

systemctl enable vlejs >/dev/null 2>&1
systemctl start vlejs >/dev/null 2>&1
systemctl restart vlejs >/dev/null 2>&1

systemctl enable trojs >/dev/null 2>&1
systemctl start trojs >/dev/null 2>&1
...
#LIMIT QUOTA
wget -q -O /usr/local/sbin/quota "https://raw.githubusercontent.com/ianlunatix/lunatix_everbody/main/limit/quota.sh"
...
#SERVICE QUOTA VMESS
cat >/etc/systemd/system/vme_quota.service << EOF
[Unit]
Description=My
...
systemctl restart vme_quota
systemctl enable vme_quota

#SERVICE QUOTAVLESS
cat >/etc/systemd/system/vle_quota.service << EOF
[Unit]
Description=My 
ProjectAfter=network.target
...
EOF
systemctl daemon-reload
systemctl restart vle_quota
systemctl enable vle_quota

#SERVICE TROJAN
cat >/etc/systemd/system/tro_quota.service << EOF
[Unit]
Description=My 
ProjectAfter=network.target
...
EOF
systemctl daemon-reload
systemctl restart tro_quota
systemctl enable tro_quota
